...
async function review(): Promise<void> {
  try {
    ...
    const diffContent = fs.existsSync('pr-diff.txt')
      ? fs.readFileSync('pr-diff.txt', 'utf8')
      : 'No diff found.';
...
${diffContent.substring(0, 50000)} // truncate to avoid token limits if too large`;
    ...
    const responseText =
      aiResponse.text ||
      'I have reviewed this PR. Looks good from a quick automated check, but human review is recommended.';
...
review().catch((err) => {
  console.error('Unhandled error in review:', err);
  process.exit(1);
});
