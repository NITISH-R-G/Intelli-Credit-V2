        ...
        <div key={i} className="border border-zinc-800 bg-[#0a0a0a] p-3 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 max-h-40">
            ...
            {c.label === 'Collateral' &&
              'assets' in c.data &&
              c.data.assets &&
              (c.data.assets as { assetType: string; declaredValue: string | number; estLiquidationValue: string | number }[]).length > 0 && (
                <div className="mt-3 space-y-2 border-t border-zinc-800 pt-2 shrink-0">
                  ...
                  {(c.data.assets as { assetType: string; declaredValue: string | number; estLiquidationValue: string | number }[]).map((asset, idx) => (
                    <div
                      key={idx}
