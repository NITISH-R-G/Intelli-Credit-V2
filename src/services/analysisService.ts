...
export const evaluateAnomalyFraudRules = (parsedData: any, metrics: any) => {
  const fraudFlags: string[] = [];
  let forceReject = false;
  let forceRefer = false;

  if (metrics.latestRevenue > 0 && metrics.latestProfit > metrics.latestRevenue) {
    fraudFlags.push("Profit exceeds revenue (Impossible state)");
    forceReject = true;
  }
  if (metrics.latestDebt > metrics.latestAssets * 2) {
    fraudFlags.push("Extreme leverage detected");
    forceRefer = true;
  }
  if (metrics.latestCashflow === 0 && metrics.latestRevenue > 1000000) {
  ...
  if (age <= 1 && metrics.latestRevenue > 100000000) { // > 10 Cr for < 1 year old company
    fraudFlags.push("Unusually high revenue for a newly established entity");
    forceRefer = true;
  }

  if (parsedData.structuredData?.revenue && parsedData.structuredData.revenue.length >= 2) {
    const prevRevenue = parsedData.structuredData.revenue[parsedData.structuredData.revenue.length - 2].value;
    if (prevRevenue > 0 && metrics.latestRevenue / prevRevenue > 5) { // > 500% growth
      fraudFlags.push("Extreme revenue growth detected (>500%)");
      forceRefer = true;
    }
  }

  if (metrics.latestRevenue > 100000000 && metrics.profitMargin < 0.01) { // > 10 Cr with < 1% profit
  ...
  if (metrics.latestRevenue > 50000000 && employeeCount > 0 && employeeCount < 5) { // > 5 Cr with < 5 employees
...
export const evaluateShellCompanyPenalties = (parsedData: any) => {
  let penaltyScore = 0;
  const shellFlags: string[] = [];
  ...
  if (shellIndicators.some((f: any) => f.status === 'Fail')) penaltyScore += 30;
  else if (shellIndicators.some((f: any) => f.status === 'Warning')) penaltyScore += 15;
  ...
  if (parsedData.shellCompanyAnalysis) {
    ...
    if (sca.isPotentialShell) {
      if (sca.riskLevel === 'High') penaltyScore += 25;
      else if (sca.riskLevel === 'Medium') penaltyScore += 15;
    }
    ...
    if (sca.indicators && sca.indicators.length > 0) {
      ...
      penaltyScore += (scaFails * 15);
      penaltyScore += (scaWarnings * 5);

      sca.indicators.forEach((ind: any) => {
        if (ind.status === 'Fail' || ind.status === 'Warning') {
          shellFlags.push(`Shell Indicator (${ind.status}): ${ind.name} - ${ind.details}`);
        }
      });
    }
    ...
    if (evidenceStr.includes('no physical assets') || evidenceStr.includes('lack of assets')) {
      penaltyScore += 15;
      shellFlags.push("Shell Indicator: Lack of physical assets on balance sheet");
    }
    if (evidenceStr.includes('virtual office') || evidenceStr.includes('registered office only')) {
      penaltyScore += 15;
      shellFlags.push("Shell Indicator: Official filings mention virtual/registered office only");
    }
  }

  return { penaltyScore, shellFlags };
};


export const evaluateUnstructuredDataPenalties = (parsedData: any) => {
  let penaltyScore = 0;
  const unstructuredFlags: string[] = [];
  ...
  if (parsedData.directorShareholderHistory) {
    ...
    if (dsh.hasRapidChanges) {
      if (dsh.riskLevel === 'High') penaltyScore += 25;
      else if (dsh.riskLevel === 'Medium') penaltyScore += 15;
      unstructuredFlags.push(`History Alert: Rapid or unexplained changes in directors/shareholders detected (${dsh.riskLevel} Volatility)`);
    }
  }
  ...
  if (siteVisitNotes.includes('no physical operations') || siteVisitNotes.includes('closed') || siteVisitNotes.includes('virtual office')) {
    penaltyScore += 25;
    unstructuredFlags.push("Primary Insight: Lack of demonstrable physical operations at site");
  }
  ...
  if (mcaStatus.includes('dormant') || mcaStatus.includes('struck off') || mcaStatus.includes('inactive')) {
    penaltyScore += 40;
    unstructuredFlags.push(`External Intelligence: Critical MCA Status (${parsedData.externalIntelligence.mcaStatus})`);
  }
  ...
  if (legalNotes.includes('fraud') || legalNotes.includes('money laundering') || legalNotes.includes('scam')) {
    penaltyScore += 35;
    unstructuredFlags.push("External Intelligence: Critical legal disputes involving fraud/money laundering");
  }
  ...
  if (shareholding.includes('opaque') || shareholding.includes('complex') || shareholding.includes('shell')) {
    penaltyScore += 20;
    unstructuredFlags.push("Unstructured Insight: Opaque or complex shareholding pattern");
  }
  ...
  if (boardNotes.includes('related party') || boardNotes.includes('unusual') || boardNotes.includes('deviation')) {
    penaltyScore += 15;
    unstructuredFlags.push("Unstructured Insight: Unusual board meeting notes or related party transactions mentioned");
  }
  ...
  if (interviewNotes.includes('evasive') || interviewNotes.includes('contradictory') || interviewNotes.includes('unclear')) {
    penaltyScore += 20;
    unstructuredFlags.push("Primary Insight: Evasive or contradictory management responses");
  }
  ...
  if (ratingReport.includes('downgrade') || ratingReport.includes('negative') || ratingReport.includes('default')) {
    penaltyScore += 30;
    unstructuredFlags.push("Unstructured Insight: Negative rating agency report or downgrade");
  }
  ...
  if (newsNotes.includes('scandal') || newsNotes.includes('investigation') || newsNotes.includes('fraud') || newsNotes.includes('crisis')) {
    penaltyScore += 25;
    unstructuredFlags.push("External Intelligence: Negative news or sector-wide crisis detected");
  }

  return { penaltyScore, unstructuredFlags };
};
...
export const calculateRiskAndFraud = (parsedData: any): CreditAnalysis => {
    ...
    const anomalyResult = evaluateAnomalyFraudRules(parsedData, anomalyMetrics);
    fraudFlags.push(...anomalyResult.fraudFlags);
    if (anomalyResult.forceReject) riskScore += 50;
    if (anomalyResult.forceRefer) riskScore += 20;
    ...
    const shellResult = evaluateShellCompanyPenalties(parsedData);
    riskScore += shellResult.penaltyScore;
    fraudFlags.push(...shellResult.shellFlags);

    // Unstructured Data Penalty
    const unstructuredResult = evaluateUnstructuredDataPenalties(parsedData);
    riskScore += unstructuredResult.penaltyScore;
    fraudFlags.push(...unstructuredResult.unstructuredFlags);
