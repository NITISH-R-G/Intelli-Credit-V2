const handleSearchCases = async (args: any, apiKey: string | undefined) => {
  if (!apiKey) {
...
const handleFetchDirectorCibil = async (args: any, apiMode: boolean, bureauApiKey: string) => {
...
const handleCalculateLtv = async (args: any, apiMode: boolean, bureauApiKey: string) => {
  if (apiMode && !bureauApiKey) {
    return { error: "Bureau API Key is missing. Please provide a key in the settings panel for Real API mode." };
  }
  if (apiMode && bureauApiKey) {
...
const handleGetMcaInfo = async (args: any, apiMode: boolean, bureauApiKey: string) => {
  if (apiMode && bureauApiKey) {
    try {
...
export const callMcpTool = async (toolName: string, args: any, apiMode: boolean, bureauApiKey: string) => {
  ...
  try {
    ...
    if (toolName === "search_cases") {
      return await handleSearchCases(args, apiKey);
    }

    if (toolName === "fetch_director_cibil") {
      return await handleFetchDirectorCibil(args, apiMode, bureauApiKey);
    }

    if (toolName === "calculate_ltv") {
      return await handleCalculateLtv(args, apiMode, bureauApiKey);
    }

    if (toolName === "get_mca_info") {
      return await handleGetMcaInfo(args, apiMode, bureauApiKey);
    }
