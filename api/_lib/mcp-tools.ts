  ...
  try {
    ...
    if (toolName === 'fetch_director_cibil') {
      if (apiMode && !bureauApiKey) {
        return {
          error:
            'Bureau API Key is missing. Please provide a key in the settings panel for Real API mode.',
        };
      }
      if (apiMode && bureauApiKey) {
        try {
          ...
          if (!response.ok) {
            return {
              error: `Bureau API returned status ${response.status}: ${response.statusText}`,
            };
          }
    ...
    if (toolName === 'calculate_ltv') {
      ...
      if (apiMode && bureauApiKey) {
        try {
          ...
          if (!response.ok) {
            return {
              error: `LTV Calculation API returned status ${response.status}: ${response.statusText}`,
            };
          }
          return await response.json();
        } catch {
